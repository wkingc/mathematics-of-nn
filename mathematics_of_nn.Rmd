...
- For a numeric outcome a commen measure of loss is the _Mean Squared Error_, which measures the average of the squared deviations from the observed outcomes and our predictions of them @fox2008applied.

\begin{equation*}
e = \frac{1}{n}\sum_{i=1}^{n}(y - y^{\prime})^2
\end{equation*}

- There are many other loss functions as well, such as _categorical cross entropy_, which can be used to measure loss when the outcome is categorical @chollet2021deep.
...
<!--
- This plot shows how moving $W$ to the right has the affect of decreasing it's gradient.
- Alpha is the learning rate.  In more complex cases where there may be many minimums, setting this parameter well prevent the algorithm from getting stuck at a local minimum instead of the absolute minimum.
-->
...
- As a funny aside, this model using the MNIST data is better at the task of identifying digits than humans.
-->
...
- This presentation peeled back the layers of what appears to be the _magic_ of deep learning.
    ...
    - We explained that the purpose of the _activation function_ applied to each layer is to detect non-linearity.
    - We showed how after each _forward pass_ from the input to the output of the model, how the parameters in each layer are updated in the _backward pass_.
