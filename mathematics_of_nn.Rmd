...
<!--
A "radian" and a "degree" are both units used to measure angles, but a radian is defined based on the ratio of an arc length to the radius of a circle, while a degree is one of 360 equal parts of a full circle, making a radian significantly larger than a degree; essentially, radians are a more mathematically natural unit, while degrees are more convenient for everyday use due to their familiar division system.
-->
...
- The problem of efficiently calculating the gradient is solved using _backpropagation_.
    - Backpropagation works by chaining together the gradients of very simple expressions, like the affine transformations that define each layer of a Neural Network, to derive the gradient of a very complicated expression like the MSE.
    - All of the individual gradients taken together have the appearance of being taped together across the computations of a Neural Network, which is why backpropagation is sometimes called _gradient tape_.

<!--
- The nerdy side of me really wishes I could spend more time on this topic.  Backpropagation is an elegant solution to a difficult problem that deserves more attentions than this.
...
- Each cycle of a _forward pass_ followed by a _backward pass_ is called an _epoch_ @chollet2021deep.
...
- The __loss__ argument specifies how the model loss is calculated at the end of each _forward pass_.
- The __optimizer__ argument specifics the optimization algorithm used to update the parameter values for the _backward pass_.
...
- As a aside, this model using the MNIST data is better at the task of identifying handwritten digits than humans who are only correct 90% of the time.
-->
...
- This presentation peeled back the layers of what appears to be the _magic_ of deep learning.
    ...
    - We showed that the mathematical representation of each layer is just a combination of translation and transformation.
    ...
    - We concluded by presenting an example of fitting a Neural Network model using real data in Python.
